...
**`«B2»`** SVG-іконки експортовані правильно. При експорті обрана «група», а
не саме вектор.

**`«B3»`** Всі іконки з SVG-спрайту додані в HTML за допомогою тегів `<svg>` і
`<use>`

**`«B4»`** Розміри іконок взяті з макета і задані елементу `<svg>` в
HTML-файлі.
...
**`«C6»`** При ховері або фокусі, іконки повинні переходити в активну
стан - змінювати колір, якщо це зазначено в макеті.
